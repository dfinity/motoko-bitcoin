...
for (testCase in TestVectors.testCases().vals()) {
    test(
        ...
        func() {
            ...
            let computedSigHashes = testCase.sigHashes();
            assert computedSigHashes.size() == testCase.numInputs;

            for (inputIndex in computedSigHashes.keys()) {
                expect.blob(Blob.fromArray(computedSigHashes[inputIndex])).equal(Blob.fromArray(testCase.expectedSigHashes[inputIndex]));
            };
        },
    );
};
