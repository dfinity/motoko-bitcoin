...
let signTestCases : [SignTransactionTestCase] = [
  {
    ...
    destinations = [(#p2pkh "193P6LtvS4nCnkDvM9uXn1gsSRqh4aDAz7", 100000)];
...
func testSignTransaction(testCase : SignTransactionTestCase) {
  ...
  let mappedUtxos = Array.map<TestUtxo, Types.Utxo>(
    utxos,
    func(utxo) {
  ...
  let expectedOutputs = Array.map<TestTxOutput, { amount : Types.Satoshi; script : Script.Script }>(
  ...
  let ecdsaProxy = BitcoinTestTools.EcdsaProxy(
    testCase.privateKey,
    testCase.signingNonces,
  );
  switch (
    Bitcoin.createSignedP2pkhTransaction(
      #p2pkh(ecdsaProxy.p2pkhAddress()),
      ecdsaProxy,
      [],
      1,
      mappedUtxos,
